...
<body>
	...
	<div>
		...
		<a href="vyhledavani.php"><button>Vyhledani vozidel v databazi - striktne</button></a>&nbsp;
	</div>
	...
	if (!($con = mysqli_connect("localhost", "host2", "kojeneckavoda", "databaze_vozidel"))) {	//navazani spojeni
		...
		if ((isset($_POST["odesli"])) && (($_POST["spz"] != "") || ($_POST["znacka"] != "") || ($_POST["typ"] != ""))) {	//osetreni nacteni stranky
			...
			if (!($vysledek = mysqli_query($con, "SELECT * FROM vozidla WHERE spz like '$where1' and znacka like '$where2' and typ like '$where3'"))) {
